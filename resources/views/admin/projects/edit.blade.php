...
@section('content')
    <div class='m-3 text-white'>
        ...
        <form action="{{ route('admin.projects.edit', $project->slug) }}" method="POST">
            ...
            <input name="title" value="{{ old('members_num') ?? $project->members_num }}" id="members_num" type="number"
                class="form-control" placeholder="Amount of members partecipate at the project">
            <label for="" class="form-label">Budget</label>
            <input name="title" value="{{ old('budget') ?? $project->budget }}" id="budget" type="number"
                class="form-control" aria-label="Amount (to the nearest dollar)" placeholder='&euro;'>
            <label for="exampleFormControlTextarea1" class="form-label">Example textarea</label>
            <textarea name="title" value="" placeholder='Description of the project' class="form-control" id="description"
